...
// popular o elemento
div.innerHTML =
`<h3>${arrayPostagens[i].tituloMateria}<p>${arrayPostagens[i].textoMateria}</p></h3>
<a href="${arrayPostagens[i].link}" target="_blank"><img height="200px" width="250px" src="${arrayPostagens[i].imagem}">
...
// adicionar evento de destaque para a semana ao passar o cursor
    function adicionarEventoDestaque() {
      ...
      for (let i = 0; i < cells.length; i++) {
        celula[i].addEventListener('click', function() {
          ...
          // encontrar a div correspondente à data
          let divs = document.querySelectorAll('.content-item');
          for (let j = 0; j < divs.length; j++) {
            if (divs[j].getAttribute('data-date') === date) {
              // rolar a página até a div desejada
              divs[j].scrollIntoView({ behavior: 'smooth' });
              break;
            }
          }
        });
